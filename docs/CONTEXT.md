...
**Data:** 2026-06-18
**Task concluída:** 27-frontend-atividade (+ ajustes pós-27: listagem global com filtros; criação/visualização/execução na própria listagem)
**Sessão:** Frontend atividade — listagem com tags, formulário com assistente de IA, detalhe com status inline, tags e início de execução; ajuste posterior tornando a listagem global por usuário com filtros (status multi, usuário, busca textual, intervalo de data); fase 3 simplificou a criação (select único "Projeto - Demanda" das demandas atribuídas), enxugou colunas (Projeto/Demanda/Atividade, Usuário só p/ gestor), transformou o "visualizar" em dialog com abas (Descrição / Últimas execuções + total de atividades do usuário + status em chip) e adicionou play/pause de execução por linha (iniciar/encerrar com histórico e assistente de IA) — sem implementar a task 28
...
**28-frontend-execucao** — módulo de execução no frontend: timer de execução ativa, encerramento e histórico completo de execuções por atividade
