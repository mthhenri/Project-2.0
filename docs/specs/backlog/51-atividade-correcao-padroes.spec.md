...
**Correção esperada:**
- Criar `shared/src/dtos/atividade/AtividadeAlterarInternoDto.ts` (`id` + campos hoje
...
- `async alterar(dto: AtividadeAlterarInternoDto): Promise<AtividadeAlteradaDto>` — `dto.id`
...
- **`CONVENTIONS.md` / `SYSTEM.SPEC.md` §7.4** — reforçar (sem duplicar o que specs
  ...
  ✅ `alterar(dto: AtividadeAlterarInternoDto)`, deixando claro que o repositório nunca
...
2. Checagem negativa: `AtividadeRepository.alterar` **não** recebe `id: number` nem `Partial<Atividade>`.
